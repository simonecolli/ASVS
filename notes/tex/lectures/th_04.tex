...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\textwidth]{images/th_04/01.png}
...
\subsection{Analisi di dataflow}\label{subsec:dataflow_analysis}
...
Per ogni blocco $i \in nodes(G)$ del CFG, si definisce lo stato entrante
...
codice. Alcuni esempi di altre analisi viste nel corso di Linguaggi e Compilatori di dataflow includono:
\begin{itemize}
...
\begin{enumerate}
    \item \textbf{Available expressions}: $\langle \wp(AE), \supseteq, \emptyset, AE, \cap, \cup \rangle$.
    \item \textbf{Live variables}: $\langle \wp(Var), \subseteq, \emptyset, Var, \cup, \cap \rangle$.
    \item \textbf{Reaching definitions}: $\langle \wp(Def), \subseteq, \emptyset, Def, \cup, \cap \rangle$.
    \item \textbf{Very busy expressions}: $\langle \wp(BE), \supseteq, \emptyset, BE, \cup, \cap \rangle$.
\end{enumerate}
...
Sia $\langle \wp(X), \subseteq, \emptyset, X, \cup, \cap \rangle$ un
reticolo completo (\Cref{def:complete_lattice}) e $f: \wp(X) \to \wp(X)$
...
La funzione di trasferimento al join point (l'istruzione $x := y + z$) calcola il
nuovo stato:
\[

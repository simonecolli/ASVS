...
\begin{itemize}
    ...
    \item $S_{1}\cup S_{2}=\{s \mid s\in S_{1}\vee s\in S_{2}\}$ è l'unione di
    due insiemi, ovvero l'insieme di tutti gli elementi che appartengono
    ad almeno uno dei due insiemi.
    \item $S_{1}\cap S_{2}=\{s \mid s\in S_{1}\wedge s\in S_{2}\}$ è l'intersezione
...
delle parti di $S$ ha $|\wp(S)| = 2^n$ elementi.
...
\begin{esempio}{Alcuni esempi di powerset}{es:powerset_examples}
    ...
    \[\wp(\mathbb{Z}) = \{\emptyset, \{\ldots, -2, -1, 0, 1, 2,
    \ldots\}, \{0\}, \{1, 2, 3\}, \ldots\}\]
...
\begin{itemize}
    \item \textbf{Riflessività:} $\forall X_{1}\in \wp(X). X_{1}\subseteq X_{1}$
    \item \textbf{Anti-simmetria:} $\forall X_{1},X_{2}\in\wp(X).X_{1}\subseteq X_{2}\wedge X_{2}\subseteq X_{1}\Rightarrow X_{1}=X_{2}$
    \item \textbf{Transitività:} $\forall X_{1},X_{2},X_{3}\in\wp(X).X_{1}\subseteq X_{2}\wedge X_{2}\subseteq X_{3}\Rightarrow X_{1}\subseteq X_{3}$
\end{itemize}


\begin{esercizio}{Inclusione inversa su powerset}{inverse_powerset_order}

L'inclusione inversa su un powerset $\wp(X)$ è un ordine parziale?
Per verificare se $\supseteq$ è un ordine parziale su $\wp(X)$,
è necessario verificare se soddisfa le tre proprietà di un ordine parziale.

\begin{itemize}
    \item \textbf{Riflessività:} $\forall X_1 \in \wp(X) \Rightarrow
    X_1 \supseteq X_1$, indica che ogni insieme è sovrainsieme di se stesso.
    \item \textbf{Anti-simmetria:} $\forall X_1, X_2 \in \wp(X),
    ...
    \item \textbf{Transitività:} $\forall X_1, X_2, X_3 \in \wp(X),
...
l'inclusione inversa $\supseteq$ è un ordine parziale sul powerset
$\wp(X)$.

\end{esercizio}

\subsection{Poset}
\begin{definizione}{Poset}{poset}

Un insieme parzialmente ordinato (partially ordered set, poset) è una coppia
$\langle X, \sqsubseteq \rangle$ dove $X$ è un insieme (\Cref{def:set}) e $\sqsubseteq$ è
una relazione di ordine parziale (\Cref{def:partial_order}) su $X$.
\end{definizione}

\begin{nota}{Esempio di poset}{poset_example}
L'esercizio \Cref{exr:inverse_powerset_order} ha mostrato che l'inclusione
inversa su un powerset $\wp(X)$ è un ordine parziale.
Quindi $\langle X, \supseteq \rangle$ è un poset.
\end{nota}
...
\begin{itemize}
    \item $x \sqsubseteq y$
    \item $\not\exists z \in X$ tale che $x \sqsubseteq z \sqsubseteq y$
\end{itemize}
...
\begin{enumerate}
    \item $y \in X$ è un upper bound di $Y$ se $\forall y' \in Y, y' \sqsubseteq y$,
    ovvero se ogni elemento dell'insieme $Y$ è minore o uguale a $y$.
    \item $y$ è l'upper bound più piccolo tra tutti gli upper bound di $Y$, ovvero
    $\forall y' \in UB(Y). y \sqsubseteq y'$, dove $UB(Y)$ è l'insieme di
    tutti gli upper bound di $Y$.
\end{enumerate}
...
è possibile dire che:
\begin{enumerate}
    \item $y \in X$ è un lower bound di $Y$ se $\forall y' \in Y, y \sqsubseteq y'$,
    ovvero se ogni elemento dell'insieme $Y$ è maggiore o uguale a $y$.
    \item $y$ è il lower bound più grande tra tutti i lower bound di $Y$, ovvero
    $\forall y' \in LB(Y). y' \sqsubseteq y$, dove $LB(Y)$ è l'insieme di
    tutti i lower bound di $Y$.
\end{enumerate}

\end{definizione}

\begin{esercizio}{Lub e glb su powerset}{lub_glb_powerset}
Dato il Poset $\langle \wp(X), \subseteq \rangle$, e siano $S_{1},S_{2}\in \wp(X)$:
\begin{enumerate}
...
Allora, affinché $U$ contenga gli elementi di $S_1$ e $S_2$, deve valere
...
\begin{definizione}{Reticolo}{lattice}
...
Dato il poset $\langle \wp(X), \subseteq \rangle$, esso forma un
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth]{images/th_02/02.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth]{images/th_02/03.png}
...
\begin{definizione}{Reticolo completo}{complete_lattice}

Un reticolo $\langle X, \sqsubseteq, \sqcup, \sqcap \rangle$ (\Cref{def:lattice}) è detto completo se:
\begin{itemize}
    \item Ogni sottoinsieme $Y \subseteq X$ ha un lub (\Cref{def:upper_bound}) in $X$.
    \item Ogni sottoinsieme $Y \subseteq X$ ha un glb (\Cref{def:lower_bound}) in $X$.
\end{itemize}
quindi si denota come:
\[
\langle X, \sqsubseteq, \bot, \top, \sqcup, \sqcap \rangle
\]

\end{definizione}

\begin{nota}{Lub del sottoinsieme vuoto}{lub_empty_set}
Se ogni sottoinsieme $Y \subseteq X$ ha un lub in $X$, allora anche il
sottoinsieme vuoto $\emptyset$ deve avere un lub in $X$.
In questo caso il lub del sottoinsieme vuoto è l'elemento bottom $\bot$ del
reticolo.
\end{nota}

\begin{esempio}{Esempi di reticolo completo e non completo}{es:complete_lattice_example}
...
$\mathbb{Z} \cup \{+\infty, -\infty\}$.
In questo caso, il reticolo
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth]{images/th_02/04.png}
...
\begin{itemize}
    ...
    \item $\sqcup$ induce $\sqcap : \sqcap S = \sqcup \{y \mid \forall x \in S . y \sqsubseteq x\}$
    \item $\sqcap$ induce $\sqcup : \sqcup S = \sqcap \{y \mid \forall x \in S . x \sqsubseteq y\}$
\end{itemize}
...
Data una relazione $R \subseteq X \times Y$, è possibile indicare che
...
fondamentali per riuscire a modellare gli stati di un programma.
...
\begin{definizione}{Funzioni monotone, embedding ordinato e isomorfe}{monotonicity_embedding_isomorphism}
...
\begin{itemize}
    ...
    \item La funzione $f$ è isomorfa se è un embedding ordinato ed è
...
\begin{esempio}{Esempio di funzione monotona, embedding ordinato e suriettiva}{}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.6\textwidth]{images/th_02/05.png}
...
\begin{esempio}{Esempio di funzione monotona ma che non è un embedding}{}
    Data $f: \langle \wp(\mathbb{Z}), \subseteq \rangle \to \langle \mathbb{Z}, \le \rangle$,
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.6\textwidth]{images/th_02/06.png}
...
Dato un poset $\langle X, \sqsubseteq \rangle$, un sottoinsieme $S \subseteq X$ è una catena (chain) se tutti gli elementi in esso contenuti sono completamente ordinati, ovvero:
\[
...
\begin{esempio}{Esempio di catena}{chain_example}
    Dato un poset $\langle \wp(\mathbb{Z}), \subseteq \rangle$ con
    ...
    \begin{figure}[H]
        ...
        \caption{L'immagine illustra un reticolo su $\wp(\mathbb{Z})$.}
        \label{fig:th_02_07}
    \end{figure}
\end{esempio}
...
\begin{esempio}{Esempio di catena ascendente che si stabilizza}{}
Una catena ascendente infinita non strettamente crescente è una catena che si stabilizza.
Questo concetto è descritto dalla seguente formula:
\[
 \exists k \ge 0 . \forall j \ge k . l_k = l_j
\]
Questo avviene perché dopo un certo numero di passi $k$, tutti gli elementi successivi
della catena sono uguali a $l_k$. Ovvero la catena si stabilizza.
\end{esempio}

\subsection{Mappe continue}

\begin{definizione}{Mappa continua}{continuous_map}

Dati due reticoli (o poset) $\langle X, \sqsubseteq_X, \sqcup_X, \sqcap_X \rangle$ e
$\langle Y, \sqsubseteq_Y, \sqcup_Y, \sqcap_Y \rangle$ e una funzione
$f: X \to Y$ (\Cref{def:function}), essa è detta continua se per ogni catena
$C \subseteq X$ (\Cref{def:chain}) tale che $\bigsqcup_X C$ esiste, valgono
le seguenti condizioni:
\begin{itemize}
    \item $\bigsqcup_Y f(C)$ esiste;
    \item $f(\bigsqcup_X C) = \bigsqcup_Y f(C)$.
\end{itemize}

\end{definizione}

\begin{nota}{Utilità delle mappe continue}{continuous_map_usefulness}
Le mappe continue sono fondamentali nell'analisi statica perché permettono di
applicare il Teorema di Kleene per calcolare i punti fissi (fixpoints)
tramite iterazione.
\end{nota}

\subsection{Fixpoint}\label{subsec:fixpoint}

L'analisi statica riduce spesso il problema della verifica di un programma
alla ricerca della soluzione di un sistema di equazioni ricorsive.
La soluzione di tali equazioni corrisponde al concetto matematico di punto
fisso.

\begin{definizione}{Punto Fisso}{fixpoint}
Dato un insieme $X$ e una funzione $f: X \to X$:
\begin{itemize}
    \item Un elemento $x \in X$ è un punto fisso di $f$ se $f(x) = x$.
    \item L'insieme di tutti i punti fissi di $f$ è denotato come
    $Fix(f) = \{ x \mid f(x) = x \}$.
\end{itemize}
\end{definizione}

In un poset $\langle X, \sqsubseteq \rangle$, sono interessanti due tipi
particolari di punti fissi:
\begin{itemize}
    \item Least Fixpoint (lfp), ovvero il punto fisso più piccolo.
    \[ \forall y \in Fix(f) . x \sqsubseteq y \]
    \item Greatest Fixpoint (gfp), ovvero il punto fisso più grande.
    \[ \forall y \in Fix(f) . x \sqsupseteq y \]
\end{itemize}

\subsubsection{Iterazioni di una funzione}

Per calcolare un punto fisso, spesso si ricorre all'applicazione ripetuta
della funzione.

\begin{definizione}{Iterazioni}{iterations}
Le iterate di una funzione $f: X \to X$ a partire da un elemento $x$ sono
definite per induzione come:
\begin{itemize}
    \item $f^0(x) = x$
    \item $f^{n+1}(x) = f(f^n(x))$
\end{itemize}
\end{definizione}

\begin{nota}{Stabilizzazione}{stabilization}
Se l'insieme $X$ è finito, la sequenza delle iterazioni $f^0, f^1, \dots$ deve
necessariamente stabilizzarsi (raggiungere un punto fisso o entrare in un
ciclo) in al massimo $|X|$ passi. Formalmente:
\[
\forall k \ge |X| . \exists n < |X| . f^k(x) = f^n(x)
\]
...
Esistono due teoremi principali che garantiscono l'esistenza (e talvolta la
costruttibilità) dei punti fissi.
La scelta del teorema dipende dalle proprietà della funzione $f$
(\Cref{def:monotonicity_embedding_isomorphism})

\begin{teorema}{Teorema di Knaster-Tarski}{knaster_tarski}

    Sia $\langle X, \sqsubseteq, \sqcup, \sqcap \rangle$ un reticolo completo e sia
$f: X \to X$ una funzione monotona.
Allora:
\begin{enumerate}
    \item $\langle Fix(f), \sqsubseteq, \sqcup, \sqcap \rangle$ è un reticolo
    completo.
    \item Il least fixpoint (lfp) è dato dal meet di tutti i ``post-fixpoints'',
    ovvero $lfp(f) = \sqcap \{ l \mid l \sqsupseteq f(l) \}$
    \item Il greatest fixpoint (gfp) è dato dal join di tutti i
    ``pre-fixpoints'', ovvero $ gfp(f) = \bigsqcup \{ l \mid l \sqsubseteq
    f(l) \} $
\end{enumerate}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{images/th_02/08.png}
    \caption{L'immagine illustra un esempio grafico del Teorema di Knaster-Tarski.}
    \label{fig:th_02_08}
\end{figure}

\end{teorema}

\begin{teorema}{Teorema di Kleene}{kleene}
Sia $\langle X, \sqsubseteq, \bot, \top, \sqcup, \sqcap \rangle$ un reticolo
completo e sia $f: X \to X$ una funzione continua (\Cref{def:continuous_map}).
Allora $f$ ha un least fixpoint che può essere calcolato costruttivamente
come il limite della catena ascendente che parte dal bottom ($\bot$):
\[ lfp(f) = \bigsqcup_{n \ge 0} f^n(\bot) \]
\end{teorema}

\begin{nota}{Confronto tra i Teoremi}{nota:tarski_vs_kleene}

I due teoremi di punto fisso presentano differenze chiave:
\begin{itemize}
    \item Knaster-Tarski: Richiede solo la monotonia. Questo teorema è più
    generale ma non fornisce un algoritmo costruttivo (dice solo ``esiste ed
    è uguale all'intersezione di \ldots'').
    \item Kleene: Richiede la continuità (condizione più forte). Questo
    teorema fornisce un algoritmo iterativo ($f(\bot), f(f(\bot)), \dots$)
